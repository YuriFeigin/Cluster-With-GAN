...
class NDB:
    def __init__(self, training_data=None, number_of_bins=100, significance_level=0.05, z_threshold=None,
                 semi_whitening =False, whitening=False, max_dims=None, cache_folder=None):
        """
        ...
        self.semi_whitening = semi_whitening
        self.ndb_eps = 1e-6
    ...
    def construct_bins(self, training_samples, bins_file):
        ...
        if self.whitening or self.semi_whitening:
            if self.whitening:
    ...
    def evaluate(self, query_samples, model_label=None):
        ...
        """
        query_samples = query_samples.reshape([query_samples.shape[0], -1])
